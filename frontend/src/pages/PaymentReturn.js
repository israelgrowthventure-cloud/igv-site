...
const PaymentReturn = () => {
  ...
  useEffect(() => {
    ...
    if (codeRetour === 'payetest' || codeRetour === 'paye') {
      ...
      setStatus('failure');
      setPaymentData({ reference, reason: 'Payment cancelled by user' });
      toast.error('Payment cancelled');
    } else {
      setStatus('failure');
      setPaymentData({ reference, reason: 'Payment failed' });
            ...
            {paymentData?.reference && (
              <div className="bg-gray-50 rounded-lg p-4 mb-6 max-w-md mx-auto">
                <p className="text-sm text-gray-600">
                  Reference: <span className="font-mono font-semibold">{paymentData.reference}</span>
                </p>
              </div>
            )}
            <p className="text-sm text-gray-500 mb-8">
      ...
      case 'error':
        return (
          <div className="text-center">
            ...
            <h2 className="text-3xl font-bold text-gray-900 mb-4">Invalid Payment Return</h2>
            <p className="text-lg text-gray-600 mb-8">
              The payment return data is invalid or missing.
            </p>
            <button
              onClick={() => navigate('/')}
  ...
  return (
    <>
      <Helmet>
        <title>Payment Return - Israel Growth Venture</title>
